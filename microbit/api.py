...
class MicroBit():
    ...
    class Compass():
        ...
        def calibrate(self):
            self.parent.cmd("print(%s.calibrate())" % self.name)

        def heading(self):
            r = self.parent.cmd("print(%s.heading())" % self.name)
            r = int(r)
            return r
    ...
    class Radio():
        ...
        def on(self):
            self.parent.cmd("print(%s.on())" % self.name)

        def send(self, s):
            if not isinstance(s, str):
                raise RuntimeError("radio.send needs a str")
            self.parent.cmd("%s.send(\"%s\")" % (self.name, s))

        def receive(self):
            r = self.parent.cmd("print(%s.receive())" % self.name)
            r = str(r)
            return r

        def receive_bytes(self):
            r = self.parent.cmd("print(%s.receive_bytes())" % self.name)
            return r

        def config(self, arg):
            self.parent.cmd("%s.config(%s)" % (self.name, arg))
